...
def check_vinni_words(tmp_str: str) -> int:
    glasn_letters = ['а', 'о', 'и', 'у', 'е', 'ю', 'э', 'я']
    count: int = 0
    for i in range(len(tmp_str)):
        if tmp_str[i] in glasn_letters:
            count += 1 
    return count

def check_vinni_poem(tmp_str: str):
    letters_set = set(list(map(check_vinni_words, tmp_str.split(" "))))
    if len(letters_set) == 1:
        print('Парам пам-пам')
    else:
        print('Пам парам')

vinni_poem = input('Введите стихотворение: ')
check_vinni_poem(vinni_poem)
